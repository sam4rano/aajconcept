...
import { databases, storage } from './appwriteConfig'; // Import the configured Appwrite client
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const fileIds = await Promise.all(
        ['oLevelResult', 'admissionLetter', 'academicResults', 'certificateOfOrigin', 'birthCertificate'].map(async (field) => {
          if (formData[field]) {
            const response = await storage.createFile('YOUR_BUCKET_ID', 'unique()', formData[field]);
            return response.$id;
          }
          return null;
        })
      ...
      const response = await databases.createDocument(
        ...
        {
          ...formData,
          oLevelResult: fileIds[0],
          admissionLetter: fileIds[1],
          academicResults: fileIds[2],
          certificateOfOrigin: fileIds[3],
          birthCertificate: fileIds[4],
        }
      );
